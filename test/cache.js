...
describe("cache", async () => {
  ...
  it("timeout/change", () => {
    const store = cache({ ms: 1000 });
    assert.equal(store.ms, 1000);
    store.timeout(5000);
    assert.equal(store.ms, 5000);
    store[Symbol.dispose]();
  });
  ...
  it("max", () => {
    ...
    store(2, 1);
    assert.equal(store.get(2), 1);
    store(3, 1);
    assert.equal(store.get(3), 1);
    store(4, 1);
    assert.equal(store.get(4), 1);
    store(5, 1);
    assert.equal(store.get(5), 1);
  ...
  describe("dispose", () => {
    it("[Symbol.dispose]", () => {
      ...
      assert.equal(store.max, Infinity);
      assert.equal(store.ms, Infinity);
    });
    ...
    it("using Node >= 24", () => {
      ...
      assert.equal(s.get("test"), undefined);
      assert.equal(s.storage.size, 0);
      assert.equal(s.max, Infinity);
      assert.equal(s.ms, Infinity);
    });
  });
});
